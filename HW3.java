...
public class HW3 {
    ...
    private static void performTestDeleteQuestion(DatabaseHelper dbHelper) {
        System.out.println("\n🔹 Test: deleteQuestion()");
        try {
            Question q = new Question("hw3user2", "Why is my loop infinite?", "student");
            dbHelper.post(q);
            boolean deleted = dbHelper.deleteQuestion("hw3user2", "Why is my loop infinite?");

            List<Question> questions = dbHelper.getAllQuestions();
            boolean stillExists = questions.stream().anyMatch(
                x -> x.getUserName().equals("hw3user2") && x.getQuestion().equals("Why is my loop infinite?"));
    ...
    private static void performTestUpdateQuestion(DatabaseHelper dbHelper) {
        System.out.println("\n🔹 Test: updateQuestion()");
        try {
            dbHelper.post(new Question("hw3user3", "Why won't my code compile?", "student"));
            boolean updated = dbHelper.updateQuestion("Why won't my code compile?", "Oh... I forgot a semicolon.");

            List<Question> questions = dbHelper.getAllQuestions();
            boolean found = questions.stream().anyMatch(
                x -> x.getQuestion().equals("Oh... I forgot a semicolon."));
